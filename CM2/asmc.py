...
from asmc_func import* # This imports both all ASMC, DREAMzs and inverse problem-related functions
...
class Sampler:
    ...
    def __init__(self, main_dir=None,CaseStudy=0,seq = 3,ndraw=10000,thin = 1,  nCR = 3, 
                 ...
                 lik_sigma_est=False,parallel_jobs=4,jr_scale=1.0,rng_seed=123,it_b=10,jr_factor=0.2,CESSf_div=0.999996,ESSf_div=0.5,AR_min=25.0):
        ...
        if self.CaseStudy==2:   
            ...
            MCMCPar.lb=np.ones((1,45))*-1
            MCMCPar.ub=np.ones((1,45))
            ...
            DNN.gpath=Extra.main_dir+'/netG_epoch_299.pth'
            ...
            DNN.netG = Generator(nc=1, nz=3, ngf=64, gfs=3, ngpu=1,cuda=DNN.cuda, gpath=DNN.gpath)
...
#                self.MCMCPar.steps = self.MCMCPar.steps + 1
            ...
            #Calculate cumulative importance weight for current beta 


 #           next_beta=MCMCPar.beta_seq[beta_block_ind+1]
#            weight= weight + (next_beta - beta) * log_p_xnew.flatten()
...
#            print("This beta loop took %5.4f seconds." % (end_time1c - start_time1c))
        ...
        # Remove zeros from pre-allocated variables if needed
