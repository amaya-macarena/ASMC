...
#            zs = zs.cuda()
    ...
    #end_time1= time.time()
    #print("the generation takes %5.4f seconds." % (end_time1 - start_time1))   
    ...
    #end_time3= time.time()
    #print("the FS takes %5.4f seconds." % (end_time3 - start_time3))
...
#	print(found)
#	print(Iter)
#	print('init_mid',(first+last)//2 )
#	print('init_first',first)
#	print('init_last',last)

    
#	if (ind_beta< 0.9919 *len(beta_list)):
#        
#		last=ind_beta+int(0.008*len(beta_list))

	while(first<=last and not found):
		
		mid=(first+last)//2   
        
		if (mid==0): #and not mid==len(beta_list)-1):
			found=True
#			print('Lower bound(mid==0) , apply smaller step')
...
#			print('increment',betainc_list[0])

			return beta_found,inc_found,CESS_found   
#       
		if (mid==len(betainc_list)-1):
			found=True
#			print('upper bound')
			...
			CESS_found=seq * (np.sum(norm_weight*contribution_found))**2 / np.sum(norm_weight*contribution_found**2)		
#			print('Found=True, position in the beta seq',mid)
#			print('next_beta',beta_list[mid])
			return beta_found,inc_found,CESS_found
        
#		print('curr_log',curr_log_lik )
...
#		print('contribution',contribution_m)
...
def resampling(nweig,seq,X,npar):
...
#    
#    R = rng.multinomial(seq, nweights, size=1)
