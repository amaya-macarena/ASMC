...
#                print("first part of the loop %5.4f seconds." % (end_time - start_time))
            ...
            if (ESS/self.MCMCPar.seq < self.MCMCPar.ESSf_div):
                
                print('Resample')
                
                Xres = resampling(norm_weight,self.MCMCPar.seq,self.MCMCVar.X,self.MCMCPar.n)
...
#            print("This beta loop took %5.4f seconds." % (end_time1c - start_time1c))
        ...
        return self.Sequences, self.Z, self.OutDiag,  self.fx, self.MCMCPar, self.MCMCVar, beta_run, jr_seq, weig_seq, CESS_ev, increment, ESS_ev, evid_cont, evid_ev, weights_unnorm, new_weight_ev, weig_cont
